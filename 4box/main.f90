...
!-- Unit conversion
      ...
      DICH = CV1 * DICH   !! [mol/kg] to [mol/m^3]
      DICL = CV1 * DICL   !! [mol/kg] to [mol/m^3]
      DICN = CV1 * DICN   !! [mol/kg] to [mol/m^3]
      DICD = CV1 * DICD   !! [mol/kg] to [mol/m^3]
      ...
      DO2H = CV1 * DO2H   !! [mol/kg] to [mol/m^3]
      DO2N = CV1 * DO2N   !! [mol/kg] to [mol/m^3]
      DO2D = CV1 * DO2D   !! [mol/kg] to [mol/m^3]
...
!--      EP_L = R * DE_L * LF * PO_L * (PO_L / (HSC + PO_L))
         ...
         !-- 温度
         TEMDX = TEMD + (T * (TEMN - TEMD) + FHD * (TEMH - TEMD) + FLD * (TEML - TEMD)) * (DT / VOCND)

         !-- 栄養塩
         PO4HX = PO4H + ((T + FHD) * (PO4D - PO4H) + FHL * (PO4L - PO4H) - EPH) * (DT / VOCNH)
         PO4LX = PO4L + ((T + FHL) * (PO4H - PO4L) + FLN * (PO4N - PO4L) + FLD * (PO4D - PO4L) - EPL) * (DT / VOCNL)
         PO4NX = PO4N + ((T + FLN) * (PO4L - PO4N) + FND * (PO4D - PO4N)) * (DT / VOCNN)
         PO4DX = PO4D + ((T + FND) * (PO4N - PO4D) + FHD * (PO4H - PO4D) + FLD * (PO4L - PO4D) + (EPH + EPL)) * (DT / VOCND)

         !-- アルカリ度
         ALKHX = ALKH + ((T + FHD) * (ALKD - ALKH) + FHL * (ALKL - ALKH) &
         &     - (2.0D0 * RRC * RCP - RNP) * EPH) * (DT / VOCNH)
         ALKLX = ALKL + ((T + FHL) * (ALKH - ALKL) + FLN * (ALKN - ALKL) &
         &     + FLD * (ALKD - ALKL) - (2.0D0 * RRC * RCP - RNP) * EPL) * (DT / VOCNL)
         ALKNX = ALKN + ((T + FLN) * (ALKL - ALKN) + FND * (ALKD - ALKN)) * (DT / VOCNN)
         ALKDX = ALKD + ((T + FND) * (ALKN - ALKD) + FHD * (ALKH - ALKD) &
         &     + FLD * (ALKL - ALKD) + (2.0D0 * RRC * RCP - RNP) * (EPH + EPL)) * (DT / VOCND)

         !-- 全炭酸
         DICHX = DICH + ((T + FHD) * (DICD - DICH) + FHL * (DICL - DICH) &
         &      - (1.0D0 + RRC) * RCP * EPH + FAH * CV1 * K0(1) * (PCO2A - PCO2H)) * (DT / VOCNH)
         DICLX = DICL + ((T + FHL) * (DICH - DICL) + FLN * (DICN - DICL) &
         &      + FLD * (DICD - DICL) - (1.0D0 + RRC) * RCP * EPL + FAL * CV1 * K0(2) * (PCO2A - PCO2L)) * (DT / VOCNL)
         DICNX = DICN + ((T + FLN) * (DICL - DICN) + FND * (DICD - DICN) &
         &      + FAN * CV1 * K0(3) * (PCO2A - PCO2N)) * (DT / VOCNN)
         DICDX = DICD + ((T + FND) * (DICN - DICD) + FHD * (DICH - DICD) &
         &      + FLD * (DICL - DICD) + (1.0D0 + RRC) * RCP * (EPH + EPL)) * (DT / VOCND)

         !-- 酸素
         DO2HX = DO2H + ((T + FHD) * (DO2D - DO2H) + FHL * (O2SATL - DO2H) + RDO2P * EPH + FAH * (O2SATH - DO2H)) * (DT / VOCNH)

         DO2DX = DO2D + ((T + FND) * (O2SATN - DO2D) + FHD * (DO2H - DO2D) + FLD * (O2SATL - DO2D) - RDO2P * (EPH + EPL)) * (DT / VOCND)
