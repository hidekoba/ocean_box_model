...
Python port of the simple 6-box ocean biogeochemistry model
...
Run:
  python3 six_box.py
  python3 six_box.py --save-csv six_box_output.csv
  python3 six_box.py --save-history six_box_history.csv
"""
import argparse
import math
...
def chemeq_const(TEM, SAL):
    ...
    and salinity [PSU]. Ported from the two/three/four-box helper.
...
def initialize(params):
    ...
    VOCND = params["VOCN"] - VOCNP - VOCNS - VOCNL - VOCNN - VOCNM
    ...
    state = {
        "PO4P": params["PO4P0"] * CV1,
        "PO4S": params["PO4S0"] * CV1,
        "PO4L": params["PO4L0"] * CV1,
        "PO4N": params["PO4N0"] * CV1,
        "PO4M": params["PO4M0"] * CV1,
        "PO4D": params["PO4D0"] * CV1,
        "ALKP": params["ALKP0"] * CV1,
        "ALKS": params["ALKS0"] * CV1,
        "ALKL": params["ALKL0"] * CV1,
        "ALKN": params["ALKN0"] * CV1,
        "ALKM": params["ALKM0"] * CV1,
        "ALKD": params["ALKD0"] * CV1,
        "DICP": params["DICP0"] * CV1,
        "DICS": params["DICS0"] * CV1,
        "DICL": params["DICL0"] * CV1,
        "DICN": params["DICN0"] * CV1,
        "DICM": params["DICM0"] * CV1,
        "DICD": params["DICD0"] * CV1,
        "DO2P": params["DO2P0"] * CV1,
        "DO2S": params["DO2S0"] * CV1,
        "DO2L": params["DO2L0"] * CV1,
        "DO2N": params["DO2N0"] * CV1,
        "DO2M": params["DO2M0"] * CV1,
        "DO2D": params["DO2D0"] * CV1,
        "PCO2A": params["PCO2A"] / CV3,
    }

    aux = {
        "AOCNP": AOCNP, "AOCNS": AOCNS, "AOCNL": AOCNL, "AOCNN": AOCNN, "AOCNM": AOCNM,
        "VOCNP": VOCNP, "VOCNS": VOCNS, "VOCNL": VOCNL, "VOCNN": VOCNN, "VOCNM": VOCNM, "VOCND": VOCND,
...
def step(state, params, aux):
    ...
    PO4NX = state["PO4N"] + (TRAN * (state["PO4M"] - state["PO4N"]) + FLN * (state["PO4L"] - state["PO4N"]) + FND * (state["PO4D"] - state["PO4N"]) - EPN) * (DT / VOCNN)
    PO4MX = state["PO4M"] + ((TRAN + FSM) * (state["PO4S"] - state["PO4M"]) + FLM * (state["PO4L"] - state["PO4M"]) + GM * (EPS + EPL)) * (DT / VOCNM)
    PO4DX = state["PO4D"] + ((TRAN + FND) * (state["PO4N"] - state["PO4D"]) + FPD * (state["PO4P"] - state["PO4D"]) + (RGM * (EPS + EPL) + (EPP + EPN))) * (DT / VOCND)
    ...
    ALKNX = state["ALKN"] + (TRAN * (state["ALKM"] - state["ALKN"]) + FLN * (state["ALKL"] - state["ALKN"]) + FND * (state["ALKD"] - state["ALKN"]) - alk_factor * EPN) * (DT / VOCNN)
    ALKMX = state["ALKM"] + ((TRAN + FSM) * (state["ALKS"] - state["ALKM"]) + FLM * (state["ALKL"] - state["ALKM"]) + alk_factor * (GM * (EPS + EPL))) * (DT / VOCNM)
    ALKDX = state["ALKD"] + ((TRAN + FND) * (state["ALKN"] - state["ALKD"]) + FPD * (state["ALKP"] - state["ALKD"]) + alk_factor * (RGM * (EPS + EPL) + (EPP + EPN))) * (DT / VOCND)
    ...
    DICNX = state["DICN"] + (TRAN * (state["DICM"] - state["DICN"]) + FLN * (state["DICL"] - state["DICN"]) + FND * (state["DICD"] - state["DICN"]) - dic_factor * EPN) * (DT / VOCNN)
    DICMX = state["DICM"] + ((TRAN + FSM) * (state["DICS"] - state["DICM"]) + FLM * (state["DICL"] - state["DICM"]) + dic_factor * (GM * (EPS + EPL))) * (DT / VOCNM)
    DICDX = state["DICD"] + ((TRAN + FND) * (state["DICN"] - state["DICD"]) + FPD * (state["DICP"] - state["DICD"]) + dic_factor * (RGM * (EPS + EPL) + (EPP + EPN))) * (DT / VOCND)
    ...
    DO2NX = state["DO2N"] + (TRAN * (state["DO2M"] - state["DO2N"]) + FLN * (state["DO2L"] - state["DO2N"]) + FND * (state["DO2D"] - state["DO2N"]) + RO2P * EPN + FAN * (o2satv["N"] - state["DO2N"])) * (DT / VOCNN)
    DO2MX = state["DO2M"] + ((TRAN + FSM) * (state["DO2S"] - state["DO2M"]) + FLM * (state["DO2L"] - state["DO2M"]) - RO2P * (GM * (EPS + EPL))) * (DT / VOCNM)
    DO2DX = state["DO2D"] + ((TRAN + FND) * (state["DO2N"] - state["DO2D"]) + FPD * (state["DO2P"] - state["DO2D"]) - RO2P * (RGM * (EPS + EPL) + (EPP + EPN))) * (DT / VOCND)

    # Carbonate system after tracer update
    tmp_state = dict(state)
    tmp_state.update({
        "ALKP": ALKPX, "ALKS": ALKSX, "ALKL": ALKLX, "ALKN": ALKNX, "ALKM": ALKMX, "ALKD": ALKDX,
        "DICP": DICPX, "DICS": DICSX, "DICL": DICLX, "DICN": DICNX, "DICM": DICMX, "DICD": DICDX,
    })
    constants_x, carb_x = carbonate_all(tmp_state, params)

    PCO2AX = state["PCO2A"] + (
        FAP * params["CV1"] * constants_x["P"]["K0"] * (carb_x["P"]["PCO2"] - state["PCO2A"])
        + FAS * params["CV1"] * constants_x["S"]["K0"] * (carb_x["S"]["PCO2"] - state["PCO2A"])
        + FAL * params["CV1"] * constants_x["L"]["K0"] * (carb_x["L"]["PCO2"] - state["PCO2A"])
        + FAN * params["CV1"] * constants_x["N"]["K0"] * (carb_x["N"]["PCO2"] - state["PCO2A"])
    ) * (DT / params["VATM"])

    new_state = {
        "PO4P": PO4PX, "PO4S": PO4SX, "PO4L": PO4LX, "PO4N": PO4NX, "PO4M": PO4MX, "PO4D": PO4DX,
        "ALKP": ALKPX, "ALKS": ALKSX, "ALKL": ALKLX, "ALKN": ALKNX, "ALKM": ALKMX, "ALKD": ALKDX,
        "DICP": DICPX, "DICS": DICSX, "DICL": DICLX, "DICN": DICNX, "DICM": DICMX, "DICD": DICDX,
        "DO2P": DO2PX, "DO2S": DO2SX, "DO2L": DO2LX, "DO2N": DO2NX, "DO2M": DO2MX, "DO2D": DO2DX,
        "PCO2A": PCO2AX,
    }
...
def make_output(state, params, aux, tocini=None, nstep=None):
    ...
    for box, temp_key, sal_key in [
        ...
        ("N", "TEMN", "SALN"), ("M", "TEMM", "SALM"), ("D", "TEMD", "SALD"),
    ]:
...
def create_dataframe(out):
    ...
    for q in ["PO4", "DIC", "ALK", "DO2", "AOU", "CO2", "HCO3", "CO32", "PCO2"]:
        unit = {
            "PO4": "umol/kg", "DIC": "umol/kg", "ALK": "ueq/kg", "DO2": "umol/kg",
            "AOU": "umol/kg", "CO2": "umol/kg", "HCO3": "umol/kg", "CO32": "umol/kg", "PCO2": "ppmv",
        }[q]
        for box in BOXES:
            rows.append((f"{q} ({unit})", box, out.get(f"{q}_{box}_{unit.replace('/', '').replace(' ', '')}")))

    # The generic key above cannot handle ALK/PCO2 naming cleanly, so rebuild these rows explicitly.
    rows = []
    for box in BOXES:
        rows.append(("Temperature (deg C)", box, out.get(f"Temperature_{box}_degC")))
    for box in BOXES:
    ...
    for i in range(1, max_iter + 1):
        ...
        if return_history and (i == 1 or i % 100 == 0):
            ...
            history.append({
                ...
                "PO4_P_umolkg": hist_out["PO4_P_umolkg"],
                "PO4_S_umolkg": hist_out["PO4_S_umolkg"],
                "PO4_L_umolkg": hist_out["PO4_L_umolkg"],
                "PO4_N_umolkg": hist_out["PO4_N_umolkg"],
                "PO4_M_umolkg": hist_out["PO4_M_umolkg"],
